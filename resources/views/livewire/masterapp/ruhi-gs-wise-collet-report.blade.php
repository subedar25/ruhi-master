<div>
    <div class="card">
        <div class="card-body">
            <div id="ruhi-collet-anchor-gs" class="d-none" data-s2-value="{{ $gsId ?? '' }}"></div>
            <div class="d-flex flex-wrap align-items-end justify-content-between w-100">
                <form wire:submit.prevent="submit" class="form-inline flex-wrap align-items-end flex-grow-1">
                    <div class="form-group mr-3 mb-2">
                        ...
                        <input type="hidden" wire:model.live="gsId" id="ruhi-collet-hidden-gs">
                        <div wire:ignore class="d-inline-block" style="min-width: 280px;">
                            <select
                                id="ruhi-collet-select-gs"
                                class="form-control form-control-sm js-ruhi-master-select2"
                                style="width: 100%; min-width: 280px;"
                                data-s2-hidden="#ruhi-collet-hidden-gs"
                                data-s2-anchor="#ruhi-collet-anchor-gs"
                ...
                <div class="mb-2 flex-shrink-0 ml-2">
                    @if($submitted && $gsId)
                        <a
                            href="{{ route('masterapp.ruhi-reports.gs-wise-collet-report.print', ['gs' => $gsId]) }}"
    ...
    @if($submitted && $report)
        <h5 class="mb-2 mt-3">
            <span class="font-weight-bold">GS Wise Collet Report</span>
            @if($selectedGsName !== '')
        ...
        <div class="table-responsive d-inline-block w-100" style="max-width: 72rem;">
            <table class="table table-bordered table-sm mb-0 w-100" style="table-layout: fixed;">
                <thead>
                    <tr>
                        <th class="text-break py-2" style="width: 18rem;">Collet</th>
                        <th class="py-2" style="width: 14rem;">K Stone</th>
                ...
                <tbody>
                    @forelse($report['rows'] as $r)
                        <tr>
                            ...
                            <td class="text-break align-top py-2">{{ $r['kstone_name'] !== '' ? $r['kstone_name'] : '—' }}</td>
                            <td class="text-left py-2">{{ number_format((int) $r['total_quantity'], 0, '.', '') }}</td>
                            <td class="text-left py-2">{{ number_format((float) $r['weight'], 2, '.', '') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted">No collet lines for this GS (design products with item type id 3).</td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot>
