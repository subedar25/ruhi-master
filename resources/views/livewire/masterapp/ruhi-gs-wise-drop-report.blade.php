<div>
    <div class="card">
        <div class="card-body">
            <div id="ruhi-wise-drop-anchor-gs" class="d-none" data-s2-value="{{ $gsId ?? '' }}"></div>
            <div class="d-flex flex-wrap align-items-end justify-content-between w-100">
                <form wire:submit.prevent="submit" class="form-inline flex-wrap align-items-end flex-grow-1">
                    <div class="form-group mr-3 mb-2">
                        ...
                        <input type="hidden" wire:model.live="gsId" id="ruhi-wise-drop-hidden-gs">
                        <div wire:ignore class="d-inline-block" style="min-width: 280px;">
                            <select
                                id="ruhi-wise-drop-select-gs"
                                class="form-control form-control-sm js-ruhi-master-select2"
                                style="width: 100%; min-width: 280px;"
                                data-s2-hidden="#ruhi-wise-drop-hidden-gs"
                                data-s2-anchor="#ruhi-wise-drop-anchor-gs"
                ...
                <div class="mb-2 flex-shrink-0 ml-2">
                    @if($submitted && $gsId)
                        <a
                            href="{{ route('masterapp.ruhi-reports.gs-wise-drop-report.print', ['gs' => $gsId]) }}"
                            target="_blank"
                            rel="noopener"
    ...
    @if($submitted && $report)
        <h5 class="mb-2 mt-3 d-flex flex-wrap align-items-center">
            <i class="fas fa-tint text-info mr-2" title="Drop"></i>
            <span class="font-weight-bold mr-1">GS Wise Drop Report</span>
            @if($selectedGsName !== '')
        ...
        <div class="table-responsive d-inline-block w-100" style="max-width: 72rem;">
            <table class="table table-bordered table-sm mb-0 w-100">
                <thead class="thead-light">
                    <tr>
                        <th class="text-break py-2"><i class="fas fa-tint mr-1 text-info"></i> Drop</th>
                        ...
                        <th class="text-left py-2"><i class="fas fa-circle mr-1 text-secondary"></i> White</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($report['rows'] as $r)
                        <tr>
                            <td class="text-break align-top py-2">{{ $r['drop'] }}</td>
                            <td class="text-left py-2">{{ number_format((int) $r['red'], 0, '.', '') }}</td>
                            <td class="text-left py-2">{{ number_format((int) $r['green'], 0, '.', '') }}</td>
                            <td class="text-left py-2">{{ number_format((int) $r['white'], 0, '.', '') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted">No drop lines for this GS.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
