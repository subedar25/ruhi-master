<div>
    <button id="ruhiAddDesignCategoryTrigger" type="button" class="d-none" wire:click="openCreateModal"></button>

    <div class="card">
        <div class="card-body p-0">
            <div id="designCategoryTableToolbar" class="d-flex flex-wrap align-items-center gap-2 px-3 pt-3 pb-2 border-bottom">
                ...
                <table class="table table-bordered table-hover table-sm mb-0">
                    ...
                    <tbody>
                        @forelse($categories as $cat)
                            <tr class="{{ $cat->deleted_at ? 'table-danger' : '' }}">
                                <td style="width: 70px; white-space: nowrap;">{{ ($categories->firstItem() ?? 1) + $loop->index }}</td>
                                <td>
                                    {{ $cat->category_name }}
                                    @if($cat->deleted_at)
                                ...
                                <td style="min-width: 120px;">
                                    <div class="action-div master-actions">
                                        <a href="#" wire:click.prevent="openEditModal({{ $cat->id }})" class="action-icon" title="Edit">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        @if($cat->deleted_at)
                                            @if((auth()->user()?->user_type ?? '') === 'systemuser')
                                                <button type="button" class="btn btn-link p-0 action-icon text-success" title="Revert" wire:click="restoreById({{ $cat->id }})" wire:confirm="Restore this category?">
                                                    <i class="fa fa-undo"></i>
                                                </button>
                                            @endif
                                        @elseif(auth()->user()?->can('delete-ruhi-design-category'))
                                                <button type="button" class="btn btn-link p-0 action-icon text-danger" title="Delete" wire:click="deleteById({{ $cat->id }})" wire:confirm="Delete this category?">
                        ...
                        <div class="row align-items-end">
                            <div class="col-md-5">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="text" class="form-control form-control-sm @error('category_name') is-invalid @enderror" wire:model.defer="category_name" maxlength="255" required>
                                    @error('category_name') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group mb-0">
                                    <label class="mb-1">Abbreviation</label>
                                    <input type="text" class="form-control form-control-sm @error('abbreviation') is-invalid @enderror" wire:model.defer="abbreviation" maxlength="20">
    ...
    <div class="modal fade {{ $showEditModal ? 'show d-block' : '' }}" tabindex="-1" role="dialog" style="background: rgba(0,0,0,0.5)">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit design category</h5>
                    <button type="button" class="close" wire:click="closeModals"><span>&times;</span></button>
                </div>
